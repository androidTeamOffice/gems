...
async function getAllDashboardCounters() {
    try {
        const [rowsLvs] = await pool.query('SELECT count(*) as lvs FROM `leaves` where end_date>=DATE(now()); ');
        const [rowsDuty] = await pool.query('SELECT count(*) as duty FROM `schedules` where date>=DATE(now()) and time(now()) between start_time and end_time; ');
        const [rowsCourse] = await pool.query('SELECT count(*) as course FROM `course_completion` where DATE(now()) between course_from and course_to;  ');
        const [rows] = await pool.query('SELECT count(emp.id) as emps FROM `employees` as emp;  ');

        console.log("fetched lvs count");
        return [rows, rowsLvs, rowsDuty, rowsCourse];
    } catch (error) {
...
module.exports = { getAllDashboardCounters }