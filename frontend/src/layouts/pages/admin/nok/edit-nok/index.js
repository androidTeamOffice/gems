...
function EditNOK() {
  ...
  const nokData = location.state?.dbData || {};
  ...
  const [formData, setFormData] = useState({
    ...
    nok_name: nokData.ni_name,
    relation: nokData.relation,
    address: nokData.address,
    contactNo: nokData.contact_no,
  });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const hasChanges = Object.values(formData).some(
        (value, index) => value !== nokData[Object.keys(formData)[index]]
      );
      if (hasChanges) {
        response = await authAxios.post("/api/update_nok_info", formData);
        console.log("Response: " + response.status);
        console.log("NOK updated successfully:", response.data);
        Swal.fire("Updated!", "NOK updated successfully.", "success");
        handleClear();
        navigate("/admin/nok/nok-list");
      } else {
        console.log("No changes detected. Update not sent.");
        Swal.fire("Info", "No changes detected. NOK not updated.", "info");
      }
    } catch (error) {
