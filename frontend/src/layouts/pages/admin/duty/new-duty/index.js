...
function NewDuty() {
  ...
  const schema = Yup.object().shape({
    ...
    cadreSpecific: Yup.object()
      .shape({
        value: Yup.string().required("cadreSpecific is required"),
      })
      .required("cadreSpecific is Required"),
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      response = await authAxios.post("/api/add_duty", formData);
      console.log("Resposne " + response.status);
      console.log("Duty created successfully:", response.data);
      Swal.fire("Added!", "Duty Added successfully.", "success");
                    ...
                    <Grid item xs={12} sm={6}>
                      <ArgonBox mb={3}>
                        ...
                        {errors.cadreSpecific && (
                          <ErrorMessage message="Please select cadre specific." />
                        )}
                      </ArgonBox>
                    </Grid>
                    <Grid item xs={12} sm={6}>
