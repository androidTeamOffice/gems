...
function DutyList() {
  ...
  const [dataTableData, setDataTableData] = useState({
    columns: [
      ...
      { Header: "Duty Description", accessor: "dutydescription" },
      { Header: "Cadre Specific", accessor: "cadrespecific" },
      { Header: "Location", accessor: "location" },
      // { Header: "Rank", accessor: "rank" },
      { Header: "Appt", accessor: "appt" },
      { Header: "Duration", accessor: "duration" },
      { Header: "Employee Required", accessor: "empReq" },
      { Header: "Occurance in Day", accessor: "occrInDay" },
  ...
  const fetchData = async () => {
    try {
      const response = await authAxios.get("/api/dutys_list");

      const formattedData = response.data.dutys.map((item, index) => ({
        ser: index + 1,
        dutyname: item.name,
        dutydescription: item.description,
        cadrespecific: item.cadre_specific,
        location: item.location_name,
        // rank: item.rank_name,
        appt: item.appt_name,
        duration: item.duration,
        empReq: item.emp_req,
        occrInDay: item.occurance_in_day,
        action: (
  ...
  const handleDeleteConfirmation = async (itemID) => {
    try {
      await authAxios.post("/api/delete_duty", { id: itemID }); // Use template literal for clarity
      Swal.fire("Deleted!", "Duty deleted successfully.", "success");
      await fetchData();
    } catch (error) {
  ...
  const handleEditClick = async (itemID) => {
    try {
      const response = await authAxios.post("/api/duty_by_id", {
        id: itemID,
      });

      if (response.data.duty) {
        const dbData = response.data.duty;
        navigate(`/admin/duty/edit-duty/`, { state: { dbData } });
      } else {
