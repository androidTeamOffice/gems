...
function EmployeList() {
  ...
  const fetchData = async () => {
    try {
      ...
      const formattedData = response.data.employees.map((item, index) => ({
        ser: index + 1,
        armyNo: item.Army_No,
        rank: item.rank_name,
        ...
        bty: item.bty,
        loc: item.loc_name,
        medicalCategory: item.medical_status_name,
        available: item.available === "n" ? "No" : "Yes",
  ...
  const handleDeleteConfirmation = async (itemID) => {
    try {
      await authAxios.post("/api/delete_employee", { id: itemID }); // Use template literal for clarity
  ...
  const handleEditClick = async (itemID) => {
    try {
      const response = await authAxios.post("/api/employee_by_id", {
        id: itemID,
      });

      if (response.data.employee) {
        const dbData = response.data.employee;
        navigate(`/admin/employee/edit-employee`, { state: { dbData } });
      } else {
        ...
        <Card>
          ...
          >
            ...
            <Stack spacing={1} direction="row">
              {/* <Link to="/admin/employee/new-employee">
