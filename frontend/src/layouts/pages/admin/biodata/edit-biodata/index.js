...
function EditBioData() {
  ...
  const bioData = location.state?.dbData || {};

  // Define initial form data state
  const [formData, setFormData] = useState({
    armyNo: bioData.Army_No || "",
    image: bioData.image,
    rank: {
      value: bioData.rank_id,
      label: bioData.rank,
    },
    bty_id: {
      value: bioData.bty_id || "",
      label: bioData.bty || "Select BioData",
    },
    lve_circle_id: {
      value: bioData.lve_circle_id,
      label: bioData.lveCircle,
    },
    trade: {
      value: bioData.cadre_id,
      label: bioData.Trade,
    },
    medicalCategory: {
      value: bioData.medical_status_id,
      label: bioData.Med_Cat,
    },
    martialStatus: {
      value: bioData.Md_Unmd,
      label: bioData.Md_Unmd === "single" ? "Single" : "Married",
    },
    qualStatus: {
      value: bioData.qual_unqual,
      label: bioData.qual_unqual === "Qual" ? "Qual" : "Unqual",
    },

    indlName: bioData.Name,
    CNIC: bioData.CNIC_Indl,
    fatherName: bioData.Father_Name,

    bdrDist: bioData.Bdr_Dist,
    district: bioData.district,
    sect: bioData.Sect,

    bloodGp: bioData.Blood_Gp,
    clCast: bioData.Cl_Cast,
    svcBktYears: bioData.Svc_Bkt_Years,
    totalSvc: bioData.Total_Svc,
    remainingSvc: bioData.Remaining_Svc,
    DOB: bioData.DOB,
    DOE: bioData.DOE,
    DOPR: bioData.DOPR,
    TOS: bioData.TOS,
    SOS: bioData.SOS,
    civEdn: bioData.Civ_Edn,
  });
  ...
  const schema = Yup.object().shape({
    ...
    svcBktYears: Yup.string().required("Svc bracket year is required"),
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const hasChanges = Object.values(formData).some(
        (value, index) => value !== bioData[Object.keys(formData)[index]]
      );
      if (hasChanges) {
        console.log("new DOE");
        console.log(formData.DOE);
        response = await authAxios.post("/api/update_bio_data", formData, {
          headers,
        });
        console.log("Response: " + response.status);
        console.log("Bio Data updated successfully:", response.data);
        Swal.fire("Updated!", "Bio Data updated successfully.", "success");
        handleClear();
        navigate("/admin/biodata/biodata-list");
      } else {
        console.log("No changes detected. Update not sent.");
        Swal.fire("Info", "No changes detected. Bio Data not updated.", "info");
      }
    } catch (error) {
      ...
      if (error.response) {
        ...
        switch (status) {
          case 400: // Bad request
            ...
            Swal.fire("Error: " + status, data.message, "error");
  ...
  const handleDateChange = (dateString, field) => {
    ...
    if (field === "DOE") {
      ...
      // Update the totalSvc field in the formData state with the calculated total service
      setFormData({ ...formData, totalSvc: totalService, DOE: formattedDate });
    }
  };
  ...
  function calculateRemainingService(DOE, totalSvcString, svcBktYears) {
    ...
    const remainingDays = Math.floor(
      remainingServiceMs / (1000 * 60 * 60 * 24)
    );
  ...
  const getImageSource = (data) => {
    if (data && data.type === "Buffer") {
      ...
      return `data:image/${data.mimeType || "png"};base64,${base64String}`; // Use provided mimeType or default to PNG
    } else {
      // Handle other data formats (optional)
      return null; // Or provide a placeholder image source
    }
  };
    ...
    if (formData.image) {
      ...
      return (
        <ArgonBox p={3}>
          {formData.image && (
            <img
              src={formData.image} // Call function to get image source
              alt="Profile Picture"
              borderRadius="lg"
              shadow="lg"
              width={300}
