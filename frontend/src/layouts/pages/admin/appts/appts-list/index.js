...
function ApptList() {
  ...
  const [dataTableData, setDataTableData] = useState({
    columns: [
      ...
      { Header: "Appt Name", accessor: "duty_apptname" },
  ...
  const fetchData = async () => {
    try {
      const response = await authAxios.get("/api/duty_appts_list");
      const formattedData = response.data.duty_appts.map((item, index) => ({
        ser: index + 1,
        duty_apptname: item.name,
        action: (
  ...
  const handleDeleteConfirmation = async (itemID) => {
    try {
      await authAxios.post("/api/delete_duty_appt", { id: itemID }); // Use template literal for clarity
      Swal.fire("Deleted!", "Appt deleted successfully.", "success");
      await fetchData();
    } catch (error) {
  ...
  const handleEditClick = async (itemID) => {
    try {
      const response = await authAxios.post("/api/duty_appt_by_id", {
        id: itemID,
      });

      if (response.data.duty_appt) {
        const dbData = response.data.duty_appt;
        navigate(`/admin/appts/edit-appt`, { state: { dbData } });
      } else {
