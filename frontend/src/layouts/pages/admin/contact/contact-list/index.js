...
function ContactList() {
  ...
  const [dataTableData, setDataTableData] = useState({
    columns: [
      ...
      { Header: "Post Office", accessor: "postOffice" },
      { Header: "Tehsil", accessor: "tehsil" },
      { Header: "Distrcit", accessor: "district" },
  ...
  const fetchData = async () => {
    try {
      const response = await authAxios.get("/api/contacts_list");
      const formattedData = response.data.contacts.map((item, index) => ({
        ser: index + 1,
        armyNo: item.soldier_id,
        rank: item.rank_name,
          ...
          <ArgonBox display="flex" alignItems="center">
            <ArgonBox mx={2}>
              <ArgonTypography
                ...
                onClick={() => handleEditClick(item.soldier_id)}
              >
            ...
            <ArgonTypography
              ...
              color="secondary"
              sx={{ cursor: "pointer", lineHeight: 0 }}
              onClick={() => handleDeleteClick(item.soldier_id)}
            >
  ...
  const handleDeleteConfirmation = async (itemID) => {
    try {
      await authAxios.post("/api/delete_contact_address", { id: itemID }); // Use template literal for clarity
  ...
  const handleEditClick = async (itemID) => {
    try {
      const response = await authAxios.post(
        "/api/contact_address_by_soldier_id",
        {
          soldierId: itemID,
        }
      );

      if (response.data.contactAddress) {
        const dbData = response.data.contactAddress;
        navigate(`/admin/contact/edit-contact/`, { state: { dbData } });
      } else {
        ...
        <Card>
          ...
          >
            ...
            <Stack spacing={1} direction="row">
              <Link to="/admin/contact/new-contact">
