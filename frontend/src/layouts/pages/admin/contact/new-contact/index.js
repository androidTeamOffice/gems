...
function NewContact() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      response = await authAxios.post("/api/add_contact_address", formData);
      console.log("Resposne " + response.status);
      console.log("Contact Addresses created successfully:", response.data);
      Swal.fire("Added!", "Contact Addresses Added successfully.", "success");
      ...
      if (error.response) {
        ...
        switch (status) {
          case 400: // Bad request
            ...
            Swal.fire(
              "Error: " + status,
              "Contact Addresses already exists. Please check and try again.",
              "error"
            );
