...
import EditCadre from "../edit-location";
...
function LocationList() {
  ...
  const fetchData = async () => {
    try {
      ...
      const formattedData = response.data.locations.map((item, index) => ({
        ser: index + 1,
  ...
  const handleDeleteConfirmation = async (itemID) => {
    try {
      await authAxios.post("/api/delete_location", { id: itemID }); // Use template literal for clarity
  ...
  const handleEditClick = async (itemID) => {
    try {
      const response = await authAxios.post("/api/location_by_id", {
        id: itemID,
      });

      if (response.data.location) {
        const dbData = response.data.location;
        navigate(`/admin/location/edit-location/`, { state: { dbData } });
      } else {
        ...
        <Card>
          ...
          >
            ...
            <Stack spacing={1} direction="row">
              <Link to="/admin/location/new-location">
