...
function BatteryList() {
  ...
  const handleEditClick = async (itemID) => {
    try {
      const response = await authAxios.post("/api/battery_by_id", {
        id: itemID,
      });

      if (response.data.battery) {
        const dbData = response.data.battery;
        navigate(`/admin/batteries/edit-battery/`, { state: { dbData } });
      } else {
        ...
        <Card>
          ...
          >
            ...
            <Stack spacing={1} direction="row">
              <Link to="/admin/batteries/new-battery">
