...
function LeaveList() {
  ...
  const [dataTableData, setDataTableData] = useState({
    columns: [
      ...
      { Header: "Availed Leave", accessor: "availLeave" },
  ...
  const fetchData = async () => {
    try {
      const response = await authAxios.get("/api/leaves_list");
      const formattedData = response.data.leaves.map((item, index) => ({
        ser: index + 1,
        armyNo: item.army_no,
        ...
        endDate:
          item.days_end < 1 ? (
            <ArgonBadge
              ...
              color="success"
              size="xs"
              badgeContent={new Date(item.end_date).toLocaleDateString()}
              container
            />
          ) : item.days_end > 1 && item.days_end < 7 ? (
            <ArgonBadge
              variant="contained"
              color="info"
              size="xs"
        ...
        days: item.days,
        availLeave: item.avail_till_date,
        action: (
  ...
  const handleDeleteConfirmation = async (itemID) => {
    try {
      await authAxios.post("/api/delete_leave", { id: itemID }); // Use template literal for clarity
  ...
  const handleEditClick = async (itemID) => {
    try {
      const response = await authAxios.post("/api/leave_by_id", {
        id: itemID,
      });

      if (response.data.leave) {
        const dbData = response.data.leave;
        navigate(`/admin/leave/edit-leave`, {
          state: { dbData },
        });
      } else {
