...
function NewCourseCompletion() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      response = await authAxios.post("/api/add_course_completion", formData);
      console.log("Resposne " + response.status);
      console.log("Course Completion created successfully:", response.data);
      Swal.fire("Added!", "Course Completion Added successfully.", "success");
  ...
  const armynooptions = army_no.map((armyno) => ({
    ...
    label: `${armyno.Army_No} ${armyno.rank} ${armyno.Name}`,
  }));
...
export default NewCourseCompletion;
